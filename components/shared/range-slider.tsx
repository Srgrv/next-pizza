...
const RangeSlider = React.forwardRef(
  (
    ...
    // Определяем начальные значения слайдера. Если value передан как массив, используем его. В противном случае используем [min, max].
    const initialValue = Array.isArray(value) ? value : [min, max];
    // Устанавливаем начальное состояние localValues.
    const [localValues, setLocalValues] = React.useState(initialValue);

    // Используем useEffect, чтобы обновлять состояние localValues, когда изменяются пропсы min, max или value.
    React.useEffect(() => {
      setLocalValues(Array.isArray(value) ? value : [min, max]);
    }, [min, max, value]);

    // Функция для обработки изменения значений слайдера. Она обновляет локальные значения и вызывает функцию onValueChange, если она передана.
